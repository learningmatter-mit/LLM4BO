...
class BO_ARGS(MapClass):
    ...
    bb_obj: object = None  # Full objective object for Agent acquisition
...
class BayesianOptimization:
    ...
    def optimize(self,) -> tuple:
        ...
        if self.obj_max is not None:
            ...
            max_ratio = torch.reshape(self.max/self.obj_max, (1,1))

            if self.verbose >= 2: print(f"\n{os.path.basename(self.savedir)} | Max: {max_ratio.item():.4f} | Regret: {simp_reg.item():.4f} | Used {self.cost}/{self.budget} budget.\n\n")
        ...
        while self.cost < self.budget:
            ...
            for self.index in range(self.batch_size):
                if self.index == 0:
                    if self.acq_fn.upper() in ['AGENT', 'SIMPLEAGENT']:
                        # Special handling for LLM-based agent acquisition
                        # Need to get protein sequences and model predictions
                        # Get protein sequences from objective function
                        if self.bb_obj is not None and hasattr(self.bb_obj, 'get_sequences'):
                            protein_sequences = self.bb_obj.get_sequences()
                        elif hasattr(self.bb_fn, 'get_sequences'):
                            protein_sequences = self.bb_fn.get_sequences()
                        else:
                            raise AttributeError("Agent acquisition requires objective function with get_sequences() method")
                        print(protein_sequences[:5])
                        # Get model predictions and uncertainties
                        if 'ENSEMBLE' in self.mtype:
                            y_preds_full_all = self.train_predict_ensemble(self.queries_x, self.norm_y)
                            model_predictions = torch.mean(y_preds_full_all, axis=1) * self.normalizer
                            model_std = torch.std(y_preds_full_all, axis=1) * self.normalizer
                        else:
                            with torch.no_grad():
                                mu, sigma = self.surrogate.model.predict_batched_gpu(self.disc_X)
                                model_predictions = mu * self.normalizer
                                model_std = sigma * self.normalizer
                        
                        # Get protein name from objective function
                        if self.bb_obj is not None:
                            protein_name = getattr(self.bb_obj, 'protein_name', 'GB1')
                        else:
                            protein_name = getattr(self.bb_fn, 'protein_name', 'GB1')
                        
                        # Create appropriate agent acquisition function
                        if self.acq_fn.upper() == 'AGENT':
                            acq = acquisition.AcquisitionAgent(
                                self.acq_fn, self.domain, self.queries_x, self.norm_y, 
                                self.normalizer, disc_X=self.disc_X, verbose=self.verbose, 
                                xi=self.xi, seed_index=self.seed_index, save_dir=self.savedir,
                                protein_sequences=protein_sequences,
                                model_predictions=model_predictions,
                                model_std=model_std,
                                protein=protein_name,
                                cycle=max(1, self.cost // self.batch_size + 1),
                                total_cycles=max(1, self.budget // self.batch_size),
                                oracle_model=self.mtype,
                                batch_size=self.batch_size,
                                device=getattr(self, 'device', 'cpu')
                            )
                        else:  # SIMPLEAGENT
                            acq = acquisition.AcquisitionSimpleAgent(
                                self.acq_fn, self.domain, self.queries_x, self.norm_y, 
                                self.normalizer, disc_X=self.disc_X, verbose=self.verbose, 
                                xi=self.xi, seed_index=self.seed_index, save_dir=self.savedir,
                                protein_sequences=protein_sequences,
                    ...
                    elif 'ENSEMBLE' in self.mtype:
                        acq = acquisition.AcquisitionEnsemble(self.acq_fn, self.domain, self.queries_x, self.norm_y, y_preds_full_all, self.normalizer, disc_X=self.disc_X, verbose=self.verbose, xi = self.xi, seed_index = self.seed_index, save_dir = self.savedir, device=getattr(self, 'device', 'cpu'), batch_size=self.batch_size)
                    else:
                        acq = acquisition.AcquisitionGP(self.acq_fn, self.domain, self.queries_x, self.norm_y, self.surrogate.model, self.normalizer, disc_X=self.disc_X, verbose=self.verbose, xi = self.xi, seed_index = self.seed_index, save_dir = self.savedir, device=getattr(self, 'device', 'cpu'), batch_size=self.batch_size)
                        acq.get_embedding()
                    acq.get_preds(None)
                else:
                ...
                max_ratio, simp_reg = self.update_trajectory(x, acq_val, idx)
                if self.acq_fn.upper() in ['AGENT', 'SIMPLEAGENT']:
                    print(acq_val, protein_sequences[idx])
                    
                    # Calculate current cycle and summaries file path
        ...
        if self.verbose >= 1: print(f"{os.path.basename(self.savedir)} | Optimization runtime: {datetime.now() - start} | Max: {max_ratio.item():.4f} | Regret: {simp_reg.item():.4f}")
    ...
    def update_trajectory(self, x, acq_val, idx):
        ...
        if self.obj_max is not None:
            ...
            if self.cost%self.batch_size == 0:
                if self.verbose >= 2: print(f"\n{os.path.basename(self.savedir)} | Max: {max_ratio.item():.4f} | Regret: {simp_reg.item():.4f} | Used {self.cost}/{self.budget} budget.\n\n")

        return max_ratio, simp_reg
